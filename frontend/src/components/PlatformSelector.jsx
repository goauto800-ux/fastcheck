...
const PLATFORM_LABELS = {
  netflix: "Netflix", amazon: "Amazon", coinbase: "Coinbase", binance: "Binance",
  spotify: "Spotify", twitter: "Twitter/X", disney_plus: "Disney+",
};
    ...
    <div className="mb-4">
      ...
      {isOpen && (
        <div className="mt-2 p-4 rounded-xl border border-white/10 bg-[#0C0C16]" style={{boxShadow:'0 0 30px rgba(139,92,246,0.06)'}}>
          ...
          <div>
            <p className="text-[10px] text-[#00F0FF] mb-2 uppercase tracking-wider font-bold">Plateformes</p>
            <div className="flex flex-wrap gap-1.5 max-h-[220px] overflow-y-auto pr-1">
              {[...platforms.email.filter(p => p.type === 'custom'), ...platforms.email.filter(p => p.type === 'holehe')].map(p => {
